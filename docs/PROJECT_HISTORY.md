## 2026-03-30 17:40 — Worker Deploy Path Recovery
...
## 2026-03-23 21:33 — Roadmap to 100/100 + Catalog Quality Baseline
**Роль:** Windows Engineering Assistant  
**Сделано:** создан отдельный roadmap `docs/ROADMAP_100.md` с фазами `Catalog Trust -> Review Truth -> Promo Integrity -> Clinic Verification -> Product Polish`; добавлен runnable audit `apps/worker/scripts/catalog-quality-audit.ts` и npm-команда `npm --prefix apps/worker run catalog:audit`; live baseline уже зафиксирован цифрами из Turso: `3252` врачей, `641` клиника, `48` активных акций, `149` врачей без активной клиники, `1574` врачей без отзывов, `48` врачей с multi-source reviews, `22` врача с verified clinic link и `151` групп дублей врачей (`302` строк).  
**Изменены файлы:** `apps/worker/scripts/catalog-quality-audit.ts`, `apps/worker/package.json`, `docs/ROADMAP_100.md`, `docs/TODO.md`, `docs/STATE.md`, `docs/PROJECT_HISTORY.md`  
**Следующий шаг:** начать remediation по `Catalog Trust P0`: прицельно разбирать exact duplicate doctors и orphan doctors без active clinic link

## 2026-03-30 00:06 — Promo Coverage Audit For All Clinics In Current Minsk Catalog
**Роль:** Windows Engineering Assistant
**Сделано:** добавлен отдельный verified audit `apps/worker/scripts/promo-coverage-audit.ts` и npm-команда `npm --prefix apps/worker run promo:coverage`; из текущего snapshot-first каталога собран полный coverage-report по всем клиникам в Минске и выгружен в `docs/promo-clinic-coverage.md`, `docs/promo-clinic-coverage.json`, `docs/promo-clinic-coverage.csv`. Проверка подтвердила baseline: `845` клиник в текущем каталоге, `11` клиник с live promotions, `41` клиника уже матчатся к источникам из `promo-sync`, `46` клиник вообще имеют реализованный promo-source match, `354` клиники имеют site URL, но пока без promo-source match, и `445` клиник вообще без official site URL в текущем snapshot. Дополнительно вручную подтверждено, что единственные реализованные, но неактивные safe-sources — `happyderm` и `klinik`; локальный запуск показал `skipped_by_robots`, поэтому они не должны включаться в automation без отдельного решения.
**Изменены файлы:** `apps/worker/scripts/promo-coverage-audit.ts`, `apps/worker/package.json`, `docs/promo-clinic-coverage.md`, `docs/promo-clinic-coverage.json`, `docs/promo-clinic-coverage.csv`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`
**Результат/доказательство:** `npm --prefix apps/worker run promo:coverage`; `npm --prefix apps/worker run check`; локальный run `python -m apps.scrapers.main --sources happyderm klinik --output-mode file --log-level INFO` -> `skipped_by_robots`
**Следующий шаг:** брать `docs/promo-clinic-coverage.*` как source of truth для расширения promo-layer и добавлять новые клиники в promo coverage только после отдельной проверки official site + robots policy

## 2026-03-30 00:13 — Official Site Web Verify For Minsk Clinics And Promotions
**Роль:** Windows Engineering Assistant
**Сделано:** добавлен host-level internet audit `apps/worker/scripts/promo-web-verify.ts` и npm-команда `npm --prefix apps/worker run promo:web-verify`; по данным `docs/promo-clinic-coverage.json` сгруппированы `316` уникальных site hosts, после чего выполнен внешний web-check homepage + promo/news path hints. Получен baseline: `298` хостов fetchable, `232` хоста показывают promo/news/discount signal на официальном сайте, `72` хоста не показывают promo-signal в текущем web-check. Дополнительно вручную перепроверены конфликтные кейсы прямыми URL: `https://doctortut.by/sale/` и `https://nordin.by/shares` отдают страницы акций, а `https://aquaminskclinic.by/promotions/` существует в текущем web-audit как promo host; `https://mercimed.by/` подтверждает Минск-адрес, но в текущей ручной проверке явная promo-page не подтверждена.
**Изменены файлы:** `apps/worker/scripts/promo-web-verify.ts`, `apps/worker/package.json`, `docs/promo-web-verify.md`, `docs/promo-web-verify.json`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`
**Результат/доказательство:** `npm --prefix apps/worker run promo:web-verify`; `npm --prefix apps/worker run check`; direct HTTP checks for `https://doctortut.by/sale/`, `https://nordin.by/shares`, `https://aquaminskclinic.by/promotions/`, `https://mercimed.by/aktsii/`, `https://eksana.by/aktsii/`
**Следующий шаг:** использовать `docs/promo-web-verify.*` вместе с `docs/promo-clinic-coverage.*`, чтобы отличать клиники без официального promo-signal от клиник, где promo-signal уже есть на сайте, но ещё не подключен к ingestion pipeline

## 2026-03-30 00:28 — Priority Backlog For Next Promo Sources
**Роль:** Windows Engineering Assistant
**Сделано:** поверх `promo-clinic-coverage` и `promo-web-verify` собран actionable backlog `apps/worker/scripts/promo-priority-report.ts` и npm-команда `npm --prefix apps/worker run promo:priority`. Артефакты выгружены в `docs/promo-priority-report.md`, `docs/promo-priority-report.json`, `docs/promo-priority-report.csv`. Проверка дала `247` кандидатов в bucket `ready_to_add` и `28` в bucket `review_existing`. Верхние кандидаты на следующую волну расширения: `Клиника «Мерси»`, `6-я городская клиническая больница`, `РНПЦ неврологии и нейрохирургии`, `Медицинский центр «Парацельс» на Маяковского`, `«Аква-Минск Клиника»`, `1-я городская клиническая больница`, `Медицинский центр «Эксана» на Надеждинской`.
**Изменены файлы:** `apps/worker/scripts/promo-priority-report.ts`, `apps/worker/package.json`, `docs/promo-priority-report.md`, `docs/promo-priority-report.json`, `docs/promo-priority-report.csv`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`
**Результат/доказательство:** `npm --prefix apps/worker run promo:priority`; `npm --prefix apps/worker run check`
**Следующий шаг:** брать top candidates из `docs/promo-priority-report.*` и подключать их по одному после отдельной ручной проверки структуры promo-page и robots policy

## 2026-03-30 00:47 — Strict Promo vs News Verification For Minsk Clinic Sites
**Роль:** Windows Engineering Assistant
**Сделано:** усилена internet-проверка официальных сайтов, чтобы отделять `homepage signal` от реально рабочей `promo/news` страницы и отдельно различать `promotions` против `news`. Для этого обновлены `apps/worker/scripts/promo-web-verify.ts` и `apps/worker/scripts/promo-priority-report.ts`, после чего заново собраны `docs/promo-web-verify.*` и `docs/promo-priority-report.*`. Новый baseline точнее предыдущего: из `316` host-ов `76` имеют рабочую verified `promo/news` страницу, но только `41` имеют verified страницу именно категории `promotions`, ещё `35` — это `news`-only. После ужесточения shortlist стал честнее: `36` клиник попали в `ready_to_add` с verified `promotions` page, `35` ушли в bucket `verified_news_only`, `165` остались в `homepage_signal_only`, а `28` existing-source кейсов требуют review.
**Изменены файлы:** `apps/worker/scripts/promo-web-verify.ts`, `apps/worker/scripts/promo-priority-report.ts`, `docs/promo-web-verify.md`, `docs/promo-web-verify.json`, `docs/promo-priority-report.md`, `docs/promo-priority-report.json`, `docs/promo-priority-report.csv`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`, `docs/STATE.md`
**Результат/доказательство:** `npm --prefix apps/worker run promo:web-verify`; `npm --prefix apps/worker run promo:priority`; `npm --prefix apps/worker run check`
**Следующий шаг:** использовать `ready_to_add` только из verified `promotions` bucket, а `verified_news_only` и `homepage_signal_only` не считать подтвержденными акциями без дополнительной ручной проверки

## 2026-03-30 01:12 — Added Two New Official Promo Sources And Published Them
**Роль:** Windows Engineering Assistant
**Сделано:** добавлены два новых official promo-source для Минска: `gurumed` (`https://gurumed.by/sales/`) и `paracels` (`https://www.narkolog.by/sales/`). Созданы scraper-ы `apps/scrapers/scrapers/gurumed.py` и `apps/scrapers/scrapers/paracels.py`, обновлены registry/config/selectors и workflow `.github/workflows/promo-sync.yml`. Локальный scrape подтвердил новые источники без ошибок: `gurumed -> 1 promotion`, `paracels -> 2 promotions`. После этого batch был штатно загружен в live Turso backfill (`inserted=3`, `updated=2`, `errors=0`), а flush promotion channel отправил все новые события в Telegram (`claimed=3`, `sent=3`, `failed=0`, `skipped=0`). Публичная проверка канала подтвердила новые посты `2026-03-29 22:12:09Z`, `22:12:17Z`, `22:12:20Z` для `Клиника ГУРУ` и `Парацельс`.
**Изменены файлы:** `apps/scrapers/scrapers/gurumed.py`, `apps/scrapers/scrapers/paracels.py`, `apps/scrapers/scrapers/__init__.py`, `config.yaml`, `selectors.yaml`, `.github/workflows/promo-sync.yml`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`, `docs/STATE.md`
**Результат/доказательство:** `python -m apps.scrapers.main --sources gurumed paracels --output-mode file --log-level INFO`; `npm --prefix apps/worker run catalog:backfill -- --batch-file ...latest-source-batch.json --chunk-size 50`; internal flush -> `{"claimed":3,"sent":3,"failed":0,"skipped":0}`; public channel `https://t.me/medsearch_minsk/83`, `/84`, `/85`
**Следующий шаг:** продолжать expansion только по verified `promotions` shortlist и выбирать следующие 1-2 нейтральных medical sources без спорных косметологических кейсов

## 2026-03-30 01:40 — Added IdealMed And Medera After Re-Checking Official Promo Routes
**Роль:** Windows Engineering Assistant
**Сделано:** повторная internet-проверка официальных сайтов показала, что shortlist можно улучшить сильнее, чем по старому audit baseline. Были найдены и подтверждены working promo-routes у `ИдеалМед` (`/akczii-i-skidki.html`) и `Эра` (`/promotions/`), после чего созданы scraper-ы `apps/scrapers/scrapers/idealmed.py` и `apps/scrapers/scrapers/medera.py`, обновлены registry/config/selectors и локальный workflow `.github/workflows/promo-sync.yml`. Локальный scrape подтвердил `idealmed -> 9 promotions` и `medera -> 10 promotions`. Прямая запись через локальный Turso backfill path уперлась в Windows-side проблему `@libsql/client/web`, поэтому ingest был переведен на live Worker endpoint. Combined envelope для двух sources вернул `500`, после чего sources были прогнаны изолированно: `idealmed -> skipped=1`, `medera -> inserted=10, updated=1`. Live API после этой итерации подтверждает `69` total promotions, включая `19` записей для `ИдеалМед` и `Эра`. Дополнительно вручную зафиксировано, что `Мерси` сейчас выглядит в основном архивным stock-source, а `Эксана` имеет promo-route без реальных promo cards, поэтому они отложены.
**Изменены файлы:** `apps/scrapers/scrapers/idealmed.py`, `apps/scrapers/scrapers/medera.py`, `apps/scrapers/scrapers/__init__.py`, `config.yaml`, `selectors.yaml`, `.github/workflows/promo-sync.yml`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`, `docs/STATE.md`
**Результат/доказательство:** `python -m apps.scrapers.main --sources idealmed medera --output-mode file --log-level INFO`; live ingest isolated via `POST /internal/ingest/source-batch` -> `idealmed skipped=1`, `medera inserted=10 updated=1`; live verify `GET /api/v1/promotions?page=1&per_page=200` -> `69` total and `19` hits for `ИдеалМед` + `Эра`
**Следующий шаг:** либо вручную flush-ить новый promo outbox в канал после дополнительной sanity-check, либо сначала запушить workflow/code changes, чтобы эти sources вошли в scheduled automation

## 2026-03-30 03:05 — Aqua-Minsk Added Locally, But Live Ingest Is Blocked
**Роль:** Windows Engineering Assistant
**Сделано:** после повторной ручной проверки нового пользовательского списка сильнее остальных news-only кандидатов подтвердился `Аква-Минск Клиника`: у офсайта есть рабочий `https://aquaminskclinic.by/news/`, robots разрешают этот path, а на ленте нашлись свежие service-promo материалы без необходимости тянуть явный архив скидок. На этой базе создан scraper `apps/scrapers/scrapers/aquaminsk.py`; он берёт только свежие service-promo публикации и исключает поздравления, режим работы и общий информационный шум. Одновременно были повторно отвергнуты или отложены `Профимед` (не подтверждён явный promo/news route для ingestion), `Bullfinch` (news archive по сути старый, `2015-2019`) и `Элеос` (route `/akcii` живой, но текущая страница почти пустая для автоматического extract). Локальный run подтвердил `aquaminsk -> 4 promotions`. Дальше вскрылся operational blocker: direct live Worker ingest через `POST /internal/ingest/source-batch` теперь отвечает `401 ingest token is invalid`, а локальный `catalog:backfill` по-прежнему падает на Windows-side ошибке `TypeError: resp.body?.cancel is not a function`. Поэтому `aquaminsk` пока локально готов, но ещё не считается live-published.
**Изменены файлы:** `apps/scrapers/scrapers/aquaminsk.py`, `apps/scrapers/scrapers/__init__.py`, `config.yaml`, `selectors.yaml`, `.github/workflows/promo-sync.yml`, `docs/git-qa-journal.md`, `docs/PROJECT_HISTORY.md`, `docs/STATE.md`
**Результат/доказательство:** `python -m apps.scrapers.main --sources aquaminsk --output-mode file --log-level INFO` -> `promotions_found=4`; batch titles: `Инъекционные методы в косметологии...`, `Нитевой лифтинг промежности`, `Лазерное лечение геморроя`, `Лазерное удаление гигромы`; failed live ingest `401 ingest token is invalid`; failed local backfill `TypeError: resp.body?.cancel is not a function`; live verify `GET /api/v1/promotions?page=1&per_page=200` -> total still `69`, `Аква-Минск Клиника` records `0`
**Следующий шаг:** восстановить рабочий ingest path (`INGEST_SHARED_SECRET` or alternative auth) или починить Windows-local backfill path, затем повторить isolated live ingest именно для `aquaminsk`

## 2026-03-30 16:55 — Ingest/Backfill Recovered, Stale Outbox Reclaim Added, Aqua-Minsk Reached Channel
**Роль:** Windows Engineering Assistant
**Сделано:** execution map переведен из анализа в remediation. Найдена и закрыта причина `401 ingest token is invalid`: локальные worker scripts читали placeholder values из `.env.txt` и игнорировали реальные secrets из `.env.local`. Для этого добавлен shared loader `apps/worker/scripts/lib/load-root-env.ts`, а ключевые scripts (`backfill-from-batch`, `catalog-quality-audit`, `catalog-remediate`, `catalog-sanitize`, `clinic-site-health-sync`, `verify-clinic-sites`) переведены на merge-порядок `.env.txt -> .env -> .env.local`. После этого direct auth smoke к live Worker уже не даёт `401`, а доходит до `400 INVALID_BATCH`, что подтверждает валидный secret path. Второй старый blocker тоже закрыт: `catalog:backfill` снова отрабатывает на Windows, а `apps/worker/scripts/apply-migrations.mjs` исправлен так, чтобы корректно переопределять env значениями из `.env.local` и снова стабильно проходить `db:migrate`. Поверх этого сделан системный fix promo pipeline: добавлена миграция `db/migrations/0005_notification_outbox_claimed_at.sql`, `notification_outbox` получил поле `claimed_at`, а `NotificationOutboxRepository` научен reclaim-ить stale `processing` entries старше окна ожидания и сбрасывать `claimed_at` после `sent/failed`. Локальный dispatch обновлённым `PromotionChannelService` поднял зависшую `Аква-Минск` запись `Лазерное удаление гигромы` и довёл её до `status=sent`, `attempt_count=2`. Публичный Telegram channel теперь подтверждает все `4` `Аква-Минск` публикации, а live Turso хранит `4` активные/видимые `Аква-Минск` promotions. Оставшийся residual issue: live API `/api/v1/promotions` всё ещё отдает только `3` `Аква-Минск` items, хотя SQL в Turso подтверждает `4`. Дополнительно найден новый внешний blocker: `wrangler deploy` сейчас падает из-за Cloudflare account mismatch (`66f004...` токен против service account `64b387...`), поэтому worker code deploy path требует отдельного account fix.
**Изменены файлы:** `apps/worker/scripts/lib/load-root-env.ts`, `apps/worker/scripts/backfill-from-batch.ts`, `apps/worker/scripts/verify-clinic-sites.ts`, `apps/worker/scripts/catalog-quality-audit.ts`, `apps/worker/scripts/clinic-site-health-sync.ts`, `apps/worker/scripts/catalog-sanitize.ts`, `apps/worker/scripts/catalog-remediate.ts`, `apps/worker/src/repositories/notification-outbox-repository.ts`, `apps/worker/src/repositories/catalog-write-repository.ts`, `apps/worker/scripts/apply-migrations.mjs`, `db/migrations/0005_notification_outbox_claimed_at.sql`, `docs/EXECUTION_MAP.md`, `docs/STATE.md`, `docs/PROJECT_HISTORY.md`
**Результат/доказательство:** auth smoke to `POST /internal/ingest/source-batch` -> `400 INVALID_BATCH` instead of `401`; `npm --prefix apps/worker run catalog:backfill -- --batch-file ...latest-source-batch.json --chunk-size 50` -> `processed_batches=1`, `inserted=4`, `updated=1`, `errors=0`; `npm run db:migrate` -> `Migrations complete`; local dispatch via `PromotionChannelService` -> `claimed=9`, `sent=9`, `failed=0`, `skipped=0`; direct DB verify for `Аква-Минск` outbox -> all `4` rows `sent`; public `https://t.me/s/medsearch_minsk` contains all `4` `Аква-Минск` titles; live `GET /api/v1/promotions?page=1&per_page=200` -> `total=73`, but only `3` `Аква-Минск` items still visible
**Следующий шаг:** восстановить Cloudflare deploy path через правильный account/token, затем дотянуть `published_at` в promo-scraper-ы и только после этого идти в следующую волну promo expansion

## 2026-03-30 17:08 — Aqua-Minsk API Mismatch Closed As Pagination Artifact
**Роль:** Windows Engineering Assistant
**Сделано:** отдельная проверка read-side показала, что `Аква-Минск` case не является data drift. Причина была в том, что `/api/v1/promotions` ограничивает `per_page` значением `50` через `parsePositiveInt`, поэтому на общем feed `page=1` видны только `3` `Аква-Минск` акции, а 4-я попадает на `page=2`. При этом API с фильтром `clinic=akva-minsk-klinika-00cbca01bc` возвращает корректные `total=4` и все четыре items. После этого `Аква-Минск` mismatch закрыт как pagination artifact, а не как потеря данных. Одновременно поверх предыдущего инцидента с отправкой старых promo в канал ужесточена policy channel dispatch: добавлена миграция `db/migrations/0006_promotions_published_at.sql`, promotions получили `published_at`, а `PromotionChannelService` теперь пропускает в Telegram только promo с date evidence — либо актуальный `ends_at`, либо свежий `published_at`. Для `aquaminsk` scraper уже передаёт реальную дату публикации.
**Изменены файлы:** `apps/scrapers/models.py`, `apps/scrapers/scrapers/aquaminsk.py`, `apps/worker/src/types/ingest.ts`, `apps/worker/src/repositories/catalog-write-repository.ts`, `apps/worker/src/services/ingest-service.ts`, `apps/worker/src/repositories/notification-outbox-repository.ts`, `apps/worker/src/utils/promotion-status.ts`, `apps/worker/src/services/promotion-channel-service.ts`, `db/migrations/0006_promotions_published_at.sql`, `docs/EXECUTION_MAP.md`, `docs/STATE.md`, `docs/PROJECT_HISTORY.md`
**Результат/доказательство:** `GET /api/v1/promotions?clinic=akva-minsk-klinika-00cbca01bc&page=1&per_page=50` -> `total=4` with all `Аква-Минск` items; `GET /api/v1/promotions?page=1&per_page=50` -> `PAGE1_AQUA=3`; `GET /api/v1/promotions?page=2&per_page=50` -> `PAGE2_AQUA=1`; `npm --prefix apps/worker run check`; `npm run db:migrate` -> `Applied migration: 0006_promotions_published_at.sql`
**Следующий шаг:** починить Cloudflare deploy path и затем пройтись по promo-scraper-ам, чтобы как можно больше источников начали отдавать `published_at` для нового channel guard
