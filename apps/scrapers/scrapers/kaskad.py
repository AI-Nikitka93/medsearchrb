...
class KaskadScraper(BaseScraper):
    source_name = "kaskad"
    base_url = "https://kaskadclinic.by"
    allowed_seed_urls = (
        "https://kaskadclinic.by/",
        "https://kaskadclinic.by/news/",
    )
    ...
    def _extract_promotions(self, clinic_external_id: str | None) -> list[PromotionRecord]:
        ...
        for promo_url, fallback_title in self._unique_candidates(promo_candidates):
            ...
            content = soup.select_one(".news__content") or soup.select_one(".page-content")
