...
class KraviraScraper(BaseScraper):
    source_name = "kravira"
    base_url = "https://www.kravira.by"
    allowed_seed_urls = (
        "https://www.kravira.by/",
        "https://www.kravira.by/actions/",
    )
    ...
    def _extract_clinic(self) -> ClinicRecord:
        ...
        address_match = re.search(
            r"ул\.\s*([А-Яа-яA-Za-zЁё.\-\s]+,\s*\d+[A-Яа-яA-Za-z\-\/]*)",
            response.text,
            re.IGNORECASE,
        )
        address = self.normalize_space(f"Минск, ул. {address_match.group(1)}") if address_match else None

        return ClinicRecord(
            source=self.source_name,
            external_id="kravira-main",
            name="Кравира",
            url=self.base_url,
            site_url=self.base_url,
            official_directory_url=self.absolute_url("/actions/"),
    ...
    def _extract_promotions(self, clinic_external_id: str | None) -> list[PromotionRecord]:
        archive_response = self.client.get_text(self.absolute_url("/actions/"), referer=self.base_url)
        archive_soup = self.soup(archive_response.text)
        promo_urls: list[str] = []

        for link in archive_soup.select("a[href*='/actions/']"):
            href = link.get("href", "")
            absolute = self.absolute_url(href)
            if absolute.rstrip("/") == self.absolute_url("/actions").rstrip("/"):
                continue
            if "/actions/" not in absolute:
                continue
        ...
        for promo_url in self.unique_urls(promo_urls):
            ...
            promo_response = self.client.get_text(promo_url, referer=self.absolute_url("/actions/"))
