...
class GuruMedScraper(BaseScraper):
    source_name = "gurumed"
    base_url = "https://gurumed.by"
    allowed_seed_urls = (
        "https://gurumed.by/",
        "https://gurumed.by/sales/",
    )
    ...
    def _extract_clinic(self) -> ClinicRecord:
        ...
        address_match = re.search(
            r"Минск,\s*ул\.\s*Первомайская,\s*22/1",
            response.text,
            re.IGNORECASE,
        )
        address = "Минск, ул. Первомайская, 22/1" if address_match else "Минск"
        return ClinicRecord(
            source=self.source_name,
            external_id="gurumed-main",
            name="Клиника ГУРУ",
            url=self.base_url,
